        ...
        for(size_t i=0; i<vpMPs.size(); i++)
        {
            ...
            for(const auto &observation : observations)
            {
                ...
                if(!pKFi->isBad())
                {
                    ...
                    if(leftIndex != -1 && pKFi->mvuRight[get<0>(observation.second)]<0) // Monocular observation
                    {
                ...
                if(pMP)
                {
                    ...
                    if(!pFrame->mpCamera2){
                        ...
                        if(pFrame->mvuRight[i]<0)
                        {
                            ...
                            e->setMeasurement(obs);
                            double invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                            e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, vector<KeyFrame*> &vpNonEnoughOptKFs)
    {
        ...
        for(auto pMP : lLocalMapPoints)
        {
            ...
            for(const auto & observation : observations)
            {
                ...
                if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
                {
                    const int cam0Index = get<0>(observation.second);
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
    {
        ...
        for(auto pMPi : vpMPs)
        {
            ...
            for(const auto & observation : observations)
            {
                ...
                KeyFrame* pKF = observation.first;
                if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForMerge != pMainKF->mnId || !pKF->GetMapPoint(get<0>(observation.second)))
                    continue;

                //cout << "-- KF view exists" << endl;
                nEdges++;

                const cv::KeyPoint &kpUn = pKF->mvKeysUn[get<0>(observation.second)];
        ...
        for(auto & mpObsMP : mpObsMPs)
        {
            MapPoint* pMPi = mpObsMP.first;
            int numObs = mpObsMP.second;
        ...
        for(auto pMPi : vpMPs)
        {
            ...
            for(const auto & observation : observations)
            {
                ...
                KeyFrame* pKF = observation.first;
                if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForKF != pMainKF->mnId || !pKF->GetMapPoint(get<0>(observation.second)))
                    continue;
        ...
        for(KeyFrame* pKFi : vpAdjustKF)
        {
            ...
            if(dist > 1.0)
            {
                if(bShowImages)
                {
                    ...
                    for(auto & i : vpMonoMPsOpt)
                    {
                        if(!i || i->isBad())
                        {
                            continue;
                        }
                        int index = get<0>(i->GetIndexInKeyFrame(pKFi));
                        if(index < 0)
                        {
...
// TODO Check this changeindex
    ...
    void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool bLarge, bool bRecInit)
    {
        ...
        for(auto mit=mVisEdges.begin(); mit!=mVisEdges.end(); mit++)
        {
            assert(mit->second>=3);
        }
    ...
    {
        InertialOptimization(pMap->GetAllKeyFrames(), bg, ba, priorG, priorA);
    }
    ...
    void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
    {
        ...
        for (auto & mLocalOb : mLocalObs)
            pairs.push_back(mLocalOb);
        ...
        for(int i=0;i<N;i++)
        {
            ...
            if(!pKFi->mPrevKF)
            {
                Verbose::PrintMess("NO INERTIAL LINK TO PREVIOUS FRAME!!!!", Verbose::VERBOSITY_NORMAL);
                continue;
            }
            if(pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
            {
        ...
        for(auto pMP : lLocalMapPoints)
        {
            ...
            for(const auto & observation : observations)
            {
                ...
                if(!pKFi->isBad())
                {
                    ...
                    if(pKFi->mvuRight[get<0>(observation.second)]<0) // Monocular observation
                    {
                        Eigen::Matrix<double,2,1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
...
// Set Loop edges
        Edge4DoF* e_loop;
        for(auto &loopConnection : LoopConnections)
        {
            KeyFrame* pKF = loopConnection.first;
            const long unsigned int nIDi = pKF->mnId;
            const set<KeyFrame*> &spConnections = loopConnection.second;
            ...
            for(auto &spC : spConnections)
            {
                const long unsigned int nIDj = spC->mnId;
                if((nIDi!=pCurKF->mnId || nIDj!=pLoopKF->mnId) && pKF->GetWeight(spC)<minFeat)
                    continue;
